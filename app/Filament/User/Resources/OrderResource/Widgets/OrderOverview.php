...
namespace App\Filament\User\Resources\OrderResource\Widgets;
    ...
    protected function getStats(): array
    {
        [$totalOrders, $chartData, $deliveredTodayOrders, $pendingOrders, $totalRevenue] =
            Cache::remember('order-overview', 60, function () {
                ...
                // Get pending orders
                $pendingOrders = Order::where('user_id', auth()->id())->whereHas('status', fn($q) => $q->where('name', 'Processing'))->count();
                $deliveredTodayOrders = Order::where('user_id', auth()->id())->whereHas('status', fn($q) => $q->where('name', 'Delivered')->where('updated_at', '>=', now()->startOfDay()))->count();

                // Get last 7 days of order count for chart
                $orderTrends = Order::where('user_id', auth()->id())
